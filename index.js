...
var colors = {};
var messages = [];
var animals = ["Alpaca", "Penguin", "Moose", "Sloth", "Pig"];
...
function getName(){
	if(previous_nickname !== undefined){
		...
		nickname = 'Anonymous';
		var num = Math.floor(Math.random() * (animals.length - 1));
		nickname = nickname.concat(animals[num]);		
	}
	return nickname;
}
	...
	socket.on('chat message', function(msg){
		if (msg.indexOf('/nickcolor')===0){
			...
			if(input !== "" && input !== undefined && existingName(input) === 0){
      		...
      		msg = sanitize(msg);
      		msg = timestamp.format('h:mm:ss a').concat(' ' + colors[socket.id] + ' ' + nicknames[socket.id] + ' '+ msg);
      		getMessage(msg);
